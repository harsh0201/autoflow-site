...
Env vars (set in Vercel dashboard):
  ...
  SENDER_NAME   = Harsh
  LS_STARTER    = <lemon squeezy checkout url>
  LS_GROWTH     = <lemon squeezy checkout url>
  LS_SCALE      = <lemon squeezy checkout url>
  PAYPAL_ME     = https://paypal.me/YOUR_HANDLE  (fallback if LS not set)
"""
...
LS_STARTER  = os.environ.get("LS_STARTER", "")
LS_GROWTH   = os.environ.get("LS_GROWTH", "")
LS_SCALE    = os.environ.get("LS_SCALE", "")

def payment_link(plan_key: str, amount: str) -> str:
    """Priority: Lemon Squeezy → PayPal.me → mailto fallback"""
    ls_map = {"starter": LS_STARTER, "growth": LS_GROWTH, "scale": LS_SCALE}
    ls_url = ls_map.get(plan_key.lower(), "")
    if ls_url:
        return ls_url
    if PAYPAL_ME:
        amt = amount.replace("$", "").replace(",", "").replace("/month", "").strip()
        return f"{PAYPAL_ME.rstrip('/')}/{amt}USD"
    return f"mailto:{GMAIL_USER}?subject=Ready to get started — {plan_key} Plan"

def cta_label(url: str) -> str:
    if "lemonsqueezy" in url or "lemon" in url:
        return "Get started now \u2192"
    if "paypal" in url:
        return "Pay via PayPal \u0026 get started \u2192"
    return "Reply to this email to get started \u2192"
...
def build_proposal_email(data: dict, plan: dict) -> tuple:
    ...
    <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e4e4e7;">
      ...
      <tr><td style="padding:40px;">
        ...
        <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:20px;margin-bottom:32px;">
          <div style="font-size:14px;color:#15803d;font-weight:600;">\U0001f6e1\ufe0f 14-Day Results Guarantee</div>
...
def build_not_a_fit_email(data: dict) -> tuple:
    fn     = data.get("first_name", "there")
    agency = data.get("agency_name", "your agency")
...
Based on the budget you shared, our retainer plans probably aren\'t the right fit right now.

A couple of free options that might help:
1. n8n.io (self-hosted automation, free)
2. Make.com free tier — 1,000 ops/month free
...
<p>Based on the budget you shared, our retainer plans probably aren\'t the right fit right now.</p>
<p>A couple of free options that might help:</p>
<ul>
  <li><strong>n8n.io</strong> (self-hosted, free)</li>
  <li><strong>Make.com free tier</strong> — 1,000 ops/month free</li>
</ul>
